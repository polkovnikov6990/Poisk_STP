...
def analyze_bullet_holes(img_before_path, img_after_path, cm_x, cm_y):
    ...
    # 5. Поиск кругов через HoughCircles
    ...
    # 5b. Поиск контуров, если кругов мало или для дополнительной устойчивости
    # Бинаризация для поиска контуров
    _, bin_img = cv2.threshold(gray, 30, 255, cv2.THRESH_BINARY)
    ...
    for cnt in contours:
        ...
        if 0.7 < circularity < 1.2 and min_area < area < max_area:
            ...
            center = (int(x), int(y))
            # Проверяем, не дублируется ли центр с уже найденными
            if all(np.linalg.norm(np.array(center) - np.array(c)) > min_radius_px for c in bullet_centers):
                cv2.circle(blurred, center, int(radius), (255, 255, 0), 2)
                cv2.circle(blurred, center, 2, (0, 0, 255), 3)
                bullet_centers.append(center)

    stp = None
    otklonenie_x = 0
    otklonenie_y = 0
    otklonenie_cm = 0
    # --- Новый блок: рисование пошаговых отрезков ---
    if len(bullet_centers) >= 2:
        import copy
        points = copy.deepcopy(bullet_centers)
        used = set()
        # 1. Найти две ближайшие точки
        min_dist = float('inf')
        pair = (0, 1)
        ...
        used.update([pair[0], pair[1]])
        cv2.line(blurred, a, b, (0, 255, 0), 2)
        # Середина первого отрезка
        m = ((a[0]+b[0])//2, (a[1]+b[1])//2)
        cv2.circle(blurred, m, 5, (0, 255, 0), -1)
        last_point = m
        step = 2
        # 2. Поиск ближайшей к последней построенной точке
        for _ in range(len(points)-2):
            ...
            for idx, pt in enumerate(points):
                if idx in used:
                    continue
                d = np.linalg.norm(np.array(last_point) - np.array(pt))
                if d < min_dist:
                    min_dist = d
                    next_idx = idx
            if next_idx is None:
                break
            c = points[next_idx]
            cv2.line(blurred, last_point, c, (0, 255, 0), 2)
            if step == 2:
                # Точка на 1/3 отрезка last_point -> c
                m2 = (int(last_point[0] + (c[0] - last_point[0]) / 3), int(last_point[1] + (c[1] - last_point[1]) / 3))
                cv2.circle(blurred, m2, 5, (0, 255, 0), -1)
                last_point = m2
            elif step == 3:
                # Точка на 1/3 отрезка last_point -> c
                m3 = (int(last_point[0] + (c[0] - last_point[0]) / 3), int(last_point[1] + (c[1] - last_point[1]) / 3))
                cv2.circle(blurred, m3, 5, (0, 255, 0), -1)
                last_point = m3
            elif step == 4:
                # Точка на 1/4 отрезка last_point -> c (а не 1.4)
                stp_x = int(last_point[0] + (c[0] - last_point[0]) / 4)
                stp_y = int(last_point[1] + (c[1] - last_point[1]) / 4)
                stp = (stp_x, stp_y)
                cv2.circle(blurred, stp, 10, (255, 0, 255), -1)
                cv2.putText(blurred, 'STP', (stp[0]+10, stp[1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 255), 2)
                cv2.line(blurred, last_point, stp, (0, 255, 0), 2)
                last_point = stp
            used.add(next_idx)
            step += 1
    # --- Конец нового блока ---

    # Если STP не найден выше, считаем его как среднюю точку (старый способ)
    if stp is None and len(bullet_centers) >= 3:
    ...
    # Отклонение СТП в см относительно центра изображения
    center_x = width // 2
    center_y = height // 2
    dx = dy = 0
    if stp is not None:
        dx = stp[0] - center_x
        dy = stp[1] - center_y
    otklonenie_x = abs(dx / pixel_per_mm_x / 10)  # в см, модуль
    otklonenie_y = abs(dy / pixel_per_mm_y / 10)  # в см, модуль
    # Рисуем красный отрезок от центра до СТП
    if stp is not None:
        cv2.line(blurred, (center_x, center_y), stp, (0, 0, 255), 2)
        otklonenie_cm = ( (dx / pixel_per_mm_x / 10) ** 2 + (dy / pixel_per_mm_y / 10) ** 2 ) ** 0.5
        text = f"{otklonenie_cm:.2f} sm"
        mid_x = (center_x + stp[0]) // 2
        mid_y = (center_y + stp[1]) // 2
        cv2.putText(blurred, text, (mid_x, mid_y), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
    ...
    result = {
        ...
        'stp': stp,
        'otklonenie_x': otklonenie_x,
        'otklonenie_y': otklonenie_y
    }
    return result, None
